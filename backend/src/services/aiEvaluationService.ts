...
// src/services/aiEvaluationService.ts - Complete Backend Service with Accurate Sentiment Analysis
import { GoogleGenerativeAI } from '@google/generative-ai';
...
export class AIEvaluationService {
  ...
  private async makeComplexAIRequest(
    ...
    try {
      ...
      // Fallback to regular model if complex model fails
      if (retryCount === 0) {
        return this.makeAIRequest(prompt, 0);
      }
...
Be thorough and educational in your analysis.`;
  ...
  // Enhanced survey evaluation with accurate sentiment analysis
  async evaluateSurveySubmission(
    formStructure: any,
    submissionData: any
  ): Promise<SurveyEvaluation> {
    try {
      ...
      // Enhanced AI analysis with strict sentiment calculation
      const prompt = `
You are an expert sentiment analysis specialist. Analyze this survey response with extreme precision.
...
2. If responses are fully negative, return: POSITIVE=0, NEUTRAL=0, NEGATIVE=100
3. If responses are fully positive, return: POSITIVE=100, NEUTRAL=0, NEGATIVE=0
4. If responses are mixed, distribute accurately but ensure total=100%
5. Be precise and data-driven in your analysis

RESPOND IN THIS EXACT FORMAT (no deviation allowed):
...
The three sentiment values MUST add up to exactly 100. Analyze each response carefully for sentiment.`;
      ...
      try {
        ...
        if (sentimentTotal !== 100) {
          console.warn(
            `⚠️ Sentiment total is ${sentimentTotal}, should be 100. Using fallback.`
          );
          return this.getDefaultSurveyAnalysis(responses);
        }
  ...
  // Enhanced feedback evaluation with accurate sentiment analysis
  async evaluateFeedbackSubmission(
    formStructure: any,
    submissionData: any
  ): Promise<FeedbackEvaluation> {
    try {
      ...
      // Enhanced AI analysis with strict sentiment calculation
      const prompt = `
You are an expert feedback sentiment analyst. Analyze this feedback with extreme precision.
...
CRITICAL REQUIREMENTS:
1. Sentiment percentages MUST total exactly 100%
2. If feedback is fully negative, return: POSITIVE=0, NEUTRAL=0, NEGATIVE=100
3. If feedback is fully positive, return: POSITIVE=100, NEUTRAL=0, NEGATIVE=0
4. If feedback is mixed, distribute accurately but ensure total=100%
5. Analyze each piece of feedback for emotional tone and sentiment
...
The three sentiment values MUST add up to exactly 100. Be precise in sentiment analysis.`;
      ...
      switch (formType) {
        ...
        case 'survey':
          try {
            ...
            evaluation.feedback = `Survey analysis: ${surveyResults.overallSentiment.positive}% positive, ${surveyResults.overallSentiment.neutral}% neutral, ${surveyResults.overallSentiment.negative}% negative sentiment. ${surveyResults.keyMetrics.length} key metrics analyzed. ${
              surveyResults.overallSentiment.positive >= 70
                ? 'Highly positive feedback received!'
                : surveyResults.overallSentiment.positive >= 50
                  ? 'Generally positive responses.'
                  : surveyResults.overallSentiment.negative >= 70
                    ? 'Concerning negative feedback detected - requires immediate attention.'
                    : surveyResults.overallSentiment.negative >= 50
                      ? 'Significant negative feedback detected.'
                      : 'Mixed feedback with room for improvement.'
            }`;
          } catch (surveyError: any) {
            console.error('❌ Survey evaluation failed:', surveyError);
        ...
        case 'feedback':
          try {
            ...
            evaluation.feedback = `Feedback analysis: ${feedbackResults.sentimentBreakdown.positive}% positive, ${feedbackResults.sentimentBreakdown.neutral}% neutral, ${feedbackResults.sentimentBreakdown.negative}% negative sentiment. ${feedbackResults.urgencyLevel} priority level, ${feedbackResults.criticalThemes.length} themes identified. ${
              feedbackResults.urgencyLevel === 'high'
                ? '🔴 Requires immediate attention!'
                : feedbackResults.urgencyLevel === 'medium'
                  ? '🟡 Important insights for consideration.'
  ...
  private getDefaultSurveyAnalysis(responses: any[]): SurveyEvaluation {
    ...
    // Ensure default sentiment totals exactly 100%
  ...
  private getDefaultFeedbackAnalysis(content: string): FeedbackEvaluation {
    ...
    return {
      ...
      sentimentBreakdown: defaultSentiment,
      actionableInsights: hasContent
        ? [
            'Review customer feedback for service improvement opportunities',
            'Consider follow-up communication for key feedback points',
  ...
  getEvaluationCapabilities(): {
    ...
    return {
      ...
      surveyFeatures: [
        'Precise sentiment analysis with exactly 100% distribution',
        'Customer satisfaction scoring and NPS calculation',
        'Response quality assessment',
        'Key metrics identification and trend analysis',
        'Business-actionable insights generation',
        'Accurate positive/neutral/negative sentiment breakdown',
      ],
      feedbackFeatures: [
        'Comprehensive theme identification and categorization',
        'Precise sentiment breakdown totaling exactly 100%',
        'Urgency level assessment for business prioritization',
        'Actionable business recommendations',
        'Critical issue identification and severity assessment',
        'Customer experience impact analysis with accurate sentiment',
      ],
      generalFeatures: [
        'Basic content analysis and categorization',
        'General sentiment assessment',
  ...
  getEvaluationStats(): {
    ...
    return {
      ...
      enhancedFeatures: [
        'Multi-modal form type detection',
        'Context-aware field analysis',
        'Precise sentiment processing with 100% total guarantee',
        'Business-focused insights generation',
        'Comprehensive error handling and recovery',
        'Batch processing optimization',
        'Enhanced quiz evaluation with fallback mechanisms',
        'Improved AI response parsing and validation',
        'Accurate sentiment normalization for surveys and feedback',
      ],
      aiModel: 'Google Gemini 2.0 Flash-Lite',
      supportedLanguages: ['en'],
    };
