...
import { logActivity, updateFormsUsed } from '../utils/formHelpers';

// @desc    Get public form data for submission (no authentication required)
...
// @desc    Get paginated forms with search, filtering, and sorting
// @route   GET /api/forms
// @access  Private
export const getAllForms = asyncHandler(async (req: Request, res: Response) => {
  ...
  const forms = await Form.find(query).sort(sortObj).skip(skip).limit(limitNum);
...
// @desc    Get single form by ID with complete structure and settings
...
export const getFormById = asyncHandler(async (req: Request, res: Response) => {
  ...
  const settings = {
    ...
    requireEmailVerification: form.settings?.requireEmailVerification || false,
  ...
  if (
    ...
    await Form.findByIdAndUpdate(formId, {
      $set: {
        'settings.allowMultipleSubmissions': settings.allowMultipleSubmissions,
...
// @desc    Create new form with unique title validation and template support
...
// @desc    Update form data, settings, and structure with title validation
...
// @desc    Delete form permanently with all submissions and associated files
...
// @desc    Create duplicate copy of existing form with new IDs
...
// @desc    Update form title with uniqueness validation
// @route   PATCH /api/forms/:id/rename
// @access  Private
export const renameForm = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
  const { name } = req.body;
...
// @desc    Toggle form favorite status
// @route   PATCH /api/forms/:id/favorite
// @access  Private
export const toggleFormFavorite = asyncHandler(
  async (req: Request, res: Response) => {
    const userId = new mongoose.Types.ObjectId(req.user.id);
    const form = await Form.findOne({
      _id: req.params.id,
...
// @desc    Archive form (hide from main list)
...
// @desc    Move form to trash (soft delete with 30-day retention)
// @route   PATCH /api/forms/:id/trash
// @access  Private
export const trashForm = asyncHandler(async (req: Request, res: Response) => {
  const userId = new mongoose.Types.ObjectId(req.user.id);
  const form = await Form.findOne({
    _id: req.params.id,
    userId,
...
// @desc    Restore form from archive or trash
...
// @desc    Publish or unpublish form with field validation
...
// @desc    Bulk update multiple forms (archive, trash, restore, favorite)
// @route   PATCH /api/forms/bulk
// @access  Private
export const bulkUpdateForms = asyncHandler(
  async (req: Request, res: Response) => {
...
// @desc    Add label to multiple forms in bulk with validation
// @route   PATCH /api/forms/bulk/add-label
// @access  Private
export const bulkAddLabel = asyncHandler(
  async (req: Request, res: Response) => {
...
// @desc    Remove label from multiple forms in bulk
// @route   PATCH /api/forms/bulk/remove-label
// @access  Private
export const bulkRemoveLabel = asyncHandler(
  async (req: Request, res: Response) => {
    const { formIds, labelId } = req.body;
    const userId = new mongoose.Types.ObjectId(req.user.id);
...
// @desc    Delete forms that have been in trash for 30+ days
// @route   DELETE /api/forms/cleanup-trash
// @access  Private
export const cleanupTrashedForms = asyncHandler(
  async (req: Request, res: Response) => {
    const userId = new mongoose.Types.ObjectId(req.user.id);
...
// @desc    Get statistics about trashed forms and cleanup eligibility
// @route   GET /api/forms/trash-stats
// @access  Private
export const getTrashStats = asyncHandler(
  async (req: Request, res: Response) => {
    const userId = new mongoose.Types.ObjectId(req.user.id);
    const thirtyDaysAgo = new Date();
    thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
...
// @desc    Delete all submissions for a specific form with file cleanup
// @route   DELETE /api/forms/form/:formId/all
// @access  Private
export const deleteAllFormSubmissions = asyncHandler(
  async (req: Request, res: Response) => {
