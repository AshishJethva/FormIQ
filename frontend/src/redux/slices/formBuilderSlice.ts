...
const serializeForm = (formData: any): Form => {
  return {
    ...formData,
    // ✅ FIXED: Convert Date objects to ISO strings for Redux
...
const formBuilderSlice = createSlice({
  ...
  reducers: {
    ...
    updateFormSettings: (
      ...
      // If allowMultipleSubmissions is disabled, also disable email submissions
    ...
    addField: (
      ...
      if (action.payload.type === FieldType.HEADING) {
        ...
        console.log(
          '✅ Creating HEADING field (no required/helpText):',
          newField
        );
      } else {
        newField = {
        ...
        console.log(
          '✅ Creating regular field (with required/helpText):',
          newField
        );
      }
    ...
    addFieldAtIndex: (
      ...
      if (type === FieldType.HEADING) {
        // Heading fields get NO additional properties
        newField = baseField as Field;
        console.log(
          '✅ Creating HEADING field at index (no required/helpText):',
          newField
        );
      } else {
        // Other fields get required and helpText
        newField = {
          ...baseField,
          required: state.form.settings?.defaultRequiredField || false,
          helpText: '',
        ...
        console.log(
          '✅ Creating regular field at index (with required/helpText):',
          newField
        );
      }
  ...
  extraReducers: builder => {
    builder
      ...
      .addCase(publishFormAsync.fulfilled, (state, action) => {
        state.isSaving = false;
        if (state.form) {
          // ✅ FIXED: Update form publish status from response
          ...
          if (updatedData && updatedData.isPublished !== undefined) {
            ...
            // Update timestamps as ISO strings
