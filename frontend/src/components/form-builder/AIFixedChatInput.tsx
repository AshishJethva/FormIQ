...
//   const canUndo = aiUpdateState?.canUndo || false;
//   const canRedo = aiUpdateState?.canRedo || false;
  ...
  const loadHistoryStats = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        setHistoryStats({
          canUndo: response.data.data.canUndo,
          canRedo: response.data.data.canRedo,
          currentIndex: response.data.data.currentIndex,
          totalSnapshots: response.data.data.totalSnapshots,
        });
      }
    } catch (error) {
        ...
        {!isExpanded && (
          ...
          >
            ...
            <motion.button
              ...
              onClick={handleUndo}
              disabled={
                !historyStats.canUndo || isUndoing || isUpdating || isRedoing
              }
            ...
            >
              {isUndoing ? (
                ...
                <RotateCcw
                  className={`w-5 h-5 ${
                    historyStats.canUndo && !isUpdating && !isRedoing
                      ? 'text-white'
                      : 'text-gray-600'
                  }`}
                />
            ...
            <motion.button
              ...
              onClick={handleRedo}
              disabled={
                !historyStats.canRedo || isRedoing || isUpdating || isUndoing
              }
              className={`w-12 h-12 rounded-full shadow-lg transition-all duration-300 flex items-center justify-center ${
                historyStats.canRedo && !isRedoing && !isUpdating && !isUndoing
            ...
            >
              {isRedoing ? (
                ...
                <RotateCw
                  className={`w-5 h-5 ${
                    historyStats.canRedo && !isUpdating && !isUndoing
                      ? 'text-white'
                      : 'text-gray-600'
                  }`}
                />
              ...
              <div className='text-xs text-gray-600 flex items-center justify-between'>
                ...
                {historyStats.totalSnapshots > 0 && (
                  <div className='flex items-center gap-1 text-xs text-gray-500'>
                    ...
                    <span>
                      {historyStats.currentIndex + 1}/
                      {historyStats.totalSnapshots}
                    </span>
                  </div>
                )}
              </div>
