...
import {
  ...
  updateHistoryState,
  clearFormHistory,
} from '@/redux/slices/formBuilder/aiFormUpdateSlice';
import { toast } from 'sonner';
import AIFixedChatInput from '@/components/form-builder/AIFixedChatInput';
import UpdateHistoryPanel from '@/components/form-builder/canvas/UpdateHistoryPanel';
import FormBuilderHeader from './navigation/FormBuilderHeader';
...
export default function FormBuilder({ formId }: FormBuilderWithAIProps) {
  ...
  const isUpdating = aiUpdateState?.isUpdating || false;
  const isUndoing = aiUpdateState?.isUndoing || false;
  const isRedoing = aiUpdateState?.isRedoing || false;
  const pathname = usePathname();
  const router = useRouter();
  ...
  useEffect(() => {
    ...
    if (
      aiUpdateState?.lastUpdateSummary &&
      !isUpdating &&
      !isUndoing &&
      !isRedoing
    ) {
  ...
  useEffect(() => {
    if (resolvedFormId) {
      dispatch(updateHistoryState(resolvedFormId));
    }
  }, [resolvedFormId, dispatch]);
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (formData.success && formData.data) {
          ...
          // Initialize or update history for the form
          if (formData.data) {
            dispatch(
              initializeHistory({
                formId: resolvedFormId,
      ...
      if (resolvedFormId) {
        try {
          ...
          if (loadFormAsync.rejected.match(result)) {
            ...
            // Initialize history for the loaded form
            dispatch(
              initializeHistory({
                formId: resolvedFormId,
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if ((e.ctrlKey || e.metaKey) && e.key === 'z' && !e.shiftKey) {
        e.preventDefault();
        if (
          aiUpdateState.canUndo &&
          !isUpdating &&
          !isUndoing &&
          resolvedFormId
        ) {
          // Dispatch undo action here if needed
      ...
      if (
        ...
        e.preventDefault();
        if (
          aiUpdateState.canRedo &&
          !isUpdating &&
          !isRedoing &&
          resolvedFormId
        ) {
          // Dispatch redo action here if needed
  ...
  // Cleanup history when component unmounts or formId changes
  useEffect(() => {
    return () => {
      // Don't clear history on unmount as user might navigate back
        ...
        <div className='text-center max-w-md'>
          ...
          <button
            onClick={() => {
              ...
              dispatch(initializeForm());
              if (resolvedFormId) {
                dispatch(clearFormHistory(resolvedFormId));
              }
            }}
            className='bg-gray-500 text-white px-4 py-2 rounded hover:bg-gray-600 cursor-pointer'
          >
  ...
  console.log('🔍 FormBuilder render - FormId resolution:', {
    ...
    isPreviewEnabled,
    historyState: {
      canUndo: aiUpdateState.canUndo,
      canRedo: aiUpdateState.canRedo,
      totalSnapshots: aiUpdateState.totalSnapshots,
      currentIndex: aiUpdateState.currentHistoryIndex,
    },
  });
  ...
  return (
    <DragProvider>
      <div className='flex flex-col h-screen bg-[#F3F3FE] overflow-hidden'>
        ...
        {/* AI Update Loading Overlay */}
        {(isUpdating || isUndoing || isRedoing) && (
          <div className='fixed inset-0 bg-black/20 backdrop-blur-sm z-50 flex items-center justify-center'>
            <div className='bg-white rounded-xl shadow-2xl p-6 flex items-center gap-4 max-w-md'>
              <div className='animate-spin rounded-full h-8 w-8 border-b-2 border-purple-600'></div>
              <div>
